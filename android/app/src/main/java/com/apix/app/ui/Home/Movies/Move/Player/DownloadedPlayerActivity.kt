...
class DownloadedPlayerActivity : AppCompatActivity() {
    ...
    private fun handleIntent(intent: Intent) {
        ...
        // Links already resolved by the APiX engine (all extensions searched).
        if (intent.action == OfflinePlaybackHelper.APIX_PLAY_ACTION) {
            // Preferred path: open now, servers stream in while the engine is still searching.
            val streaming = com.lagradost.cloudstream3.apix.player.ApixPlaybackSession.consume()
            if (streaming != null) {
                OfflinePlaybackHelper.playProgressive(this, streaming)
                return
            }
            val pending = OfflinePlaybackHelper.pendingApixPlayback
            OfflinePlaybackHelper.pendingApixPlayback = null
            if (pending != null && pending.first.isNotEmpty()) {
                OfflinePlaybackHelper.playResolved(this, pending.first, pending.second, pending.third)
                return
            }
            finishAndRemoveTask()
            return
