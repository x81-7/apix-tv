    ...
    override suspend fun generateLinks(
        ...
        links.amap { link ->
            if (!extract || !loadExtractor(link.url, refererUrl, {
                    val subtitle = PlayerSubtitleHelper.getSubtitleData(it)
                    if (subtitle.isAllowedPlaybackSubtitle()) subtitleCallback(subtitle)
                }) {
                    callback(it to null)
                }) {
...
class MinimalLinkGenerator(
    ...
    override suspend fun generateLinks(
        ...
        for (link in subs) {
            val subtitle = link.toSubtitleData()
            if (subtitle.isAllowedPlaybackSubtitle()) {
